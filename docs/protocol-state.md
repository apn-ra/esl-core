...
Upper layers that want the supported ingress surface should use
`Inbound\InboundPipeline`; that facade still preserves the same truthful reply
shapes while avoiding direct dependence on provisional classifier classes.
...
instead. If the client subscribes with XML format, `text/event-xml` frames may
arrive instead, though that path remains provisional compared with the better
evidenced plain/json paths. `esl-core` normalizes these supported formats
through the same `EventParser` → `NormalizedEvent` → `EventFactory` flow,
exposed publicly through `Inbound\InboundPipeline` when callers ingest raw bytes.

The repository now includes curated live captures proving the bridge/playback
event path in both formats. Those captures were generated by non-public smoke
tooling and serve as evidence for the parser/classifier/event-factory path;
they are not a supported runtime integration surface.
