...
| Low-level ingress composition contracts (`FrameParserInterface`, `CompletableFrameParserInterface`, `EventParserInterface`, `InboundMessageClassifierInterface`) | Advanced and intended for fixture-backed composition, not the default downstream ingress path; `InboundMessageClassifierInterface` now returns the public `ClassifiedMessageInterface` result contract |
| `Commands\*` serialization | Stable after Phase 5 |
| `Inbound\InboundPipeline`, `DecodedInboundMessage`, `InboundMessageType` | Stable for the currently supported inbound byte-stream → typed message path; `InboundPipeline::withDefaults()` is the preferred stable construction path, while `withContracts(...)` and direct constructor injection remain advanced public seams without an active soft deprecation |
...
| `Contracts\ClassifiedMessageInterface` | Stable as the public read-only classifier result contract for advanced classified-message access; the current internal carrier implements it, and custom classifier implementations may return their own public-contract implementations |
...
The support level in this table does not mean every composition seam is equally preferred. A surface may be stable while still being an advanced public seam rather than the default downstream integration path. In this release line, that distinction is intentional: `InboundPipeline::withDefaults()`, `SocketTransportFactory`, and `InboundConnectionFactory` are the preferred stable seams, while `InboundPipeline::withContracts(...)` and lower-level parser/classifier/factory composition remain public but more advanced.
