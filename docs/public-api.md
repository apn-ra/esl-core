...
`Contracts\ProvidesNormalizedSubstrateInterface` is the additive explicit contract for callers that already own a typed event and need the underlying `NormalizedEvent` without relying on concrete property names or reflection-soft coupling. `NormalizedEvent` and the built-in typed wrappers implement this contract. The built-in typed wrappers also expose a public readonly `$normalized` property in this release line as a convenience mirror of the same substrate, but downstream code that cares about the documented stable seam should prefer `DecodedInboundMessage::normalizedEvent()` or `ProvidesNormalizedSubstrateInterface`. Custom typed events that want the same richer correlation/replay substrate access must implement the interface intentionally; exposing a similarly named property alone is not a supported extension mechanism.
